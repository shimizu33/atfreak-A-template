...
function tokan_setup() {
  add_theme_support( 'title-tag' );
  add_theme_support( 'post-thumbnails' );
}
add_action( 'after_setup_theme', 'tokan_setup' );
...
function tokan_scripts() {
  // Theme stylesheet.
  wp_enqueue_style( 'tokan-style', get_stylesheet_uri() );
}
add_action( 'wp_enqueue_scripts', 'tokan_scripts' );
...
管理画面URL
https://tokan-recruit.com/wp/wp-admin/edit.php?post_type=mw-wp-form&page=mw-wp-form-save-data


【応募情報の閲覧方法】
１．管理画面へログイン
２．問合せデータページにて問合せ件数の数字部分をクリック
３．一覧のタイトル部分をクリック

【管理画面ご利用のご注意】
管理画面をご利用の際はマニュアルに記載されている動作以外を行われると、
デザイン崩れ、システムエラーが発生する場合がありますので、
ご利用の際はご注意ください。';
    return $Mail;
}
add_filter( 'mwform_admin_mail_mw-wp-form-52', 'my_admin_mail', 10, 3 );
